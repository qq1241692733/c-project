...
//********************************************************************	
	for(i1 = 0; i1 < a1; i1 ++)  //¿É»ØÊÕ a1
	{	
		if(i1 < 4)
		{
				LCD_ShowxNum(120+ i1*35,220,arr1[i1],8,24,1);    //ÏÔÊ¾	
		}else
		{
				LCD_ShowxNum(120+(i1-4)*35,244,arr1[i1],8,24,1);    //ÏÔÊ¾	
		}				
	}
//********************************************************************	
	for(i2 = 0; i2 < a2; i2++) //ÓÐº¦ a2
	{	
		if(i2 < 4)
		{
				LCD_ShowxNum(144+120+ i2*35,220,arr2[i2],8,24,1);    //ÏÔÊ¾	
		}else
		{
				LCD_ShowxNum(144+120+(i2-4)*35,244,arr2[i2],8,24,1);    //ÏÔÊ¾	
		}				
	}
//********************************************************************	
	for(i3 = 0; i3 < a3; i3++) //³øÓà a2
	{	
		if(i3 < 4)
		{
				LCD_ShowxNum(144*2+120+ i3*35,220,arr3[i3],8,24,1);    //ÏÔÊ¾	
		}else
		{
				LCD_ShowxNum(144*2+120+(i3-4)*35,244,arr3[i3],8,24,1);    //ÏÔÊ¾	
		}				
	}	
//********************************************************************	
	for(i4 = 0; i4 < a4; i4++) //
	{	
		if(i4 < 4)
		{
				LCD_ShowxNum(144*3+120+ i4*35,220,arr4[i4],8,24,1);    //ÏÔÊ¾	
		}else
		{
				LCD_ShowxNum(144*3+120+(i4-4)*35,244,arr4[i4],8,24,1);    //ÏÔÊ¾	
		}				
	}	
}
...
UPD:    
	...
	LCD_ShowString(28,85,840,16,24, "|            |           |           |           |           |"); 
	           Show_Str(28,100,840,24,"     ÏîÄ¿     ¿É»ØÊÕÀ¬»ø   ÓÐº¦À¬»ø    ³øÓàÀ¬»ø    ÆäËûÀ¬»ø",24,1);  
 
	LCD_ShowString(28,130,840,16,24,"|------------|-----------|-----------|-----------|-----------|");
	LCD_ShowString(28,145,840,16,24,"|            |           |           |           |           |"); 
          	Show_Str(28,160,840,24,"     ÊýÁ¿ ",24,1 ); 
	...
	LCD_ShowString(28,240,840,16,24,"|            |           |           |           |           |"); 	
	          Show_Str(28,232,840,24,"     ÐòºÅ ",24,1 ); 

	LCD_ShowString(28,280,840,16,24,"|------------|-----------|-----------|-----------|-----------|");
	LCD_ShowString(28,295,840,16,24,"|            |           |           |           |           |"); 
////	LCD_ShowString(28,280,840,16,24,"                  --          --          --          --      ");			
            Show_Str(28,310,840,24,"   ÂúÒç×´Ì¬       --          --          --          --      ",24,1 ); 

	LCD_ShowString(28,340,840,16,24,"|------------|-----------|-----------|-----------|-----------|"); 

 	POINT_COLOR=BLUE;
	Show_Str(360,30,200,24,"À¬»ø·ÖÀà",24,0);	
	LCD_ShowxNum(170,160,a1,8,24,0);    //ÏÔÊ¾
	LCD_ShowxNum(314,160,a2,8,24,0);    //ÏÔÊ¾
  LCD_ShowxNum(458,160,a3,8,24,0);    //ÏÔÊ¾	
	LCD_ShowxNum(602,160,a4,8,24,0);    //ÏÔÊ¾		
	ReDisplay(a1, a2, a3, a4, arr1, arr2, arr3, arr4);
	
	time_init(20000,84-1);
	
	while(1)   // PF10 CW×ªÏò    PF9 CLKÂö³å    PF4 ENÊ¹ÄÜ 
	{
		isfull();
		a=garbage_ret();
		if(a!=0)					
