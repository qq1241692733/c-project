...
void gpio_init()
{
	...
	GPIO_Initstructure.GPIO_Pin=GPIO_Pin_1|GPIO_Pin_2|GPIO_Pin_0|GPIO_Pin_4;   //Í¨ÐÅ
...
int garbage_ret()
{
	if(GPIO_ReadInputDataBit(GPIOF, GPIO_Pin_0)==1)   //¿É»ØÊÕÀ¬»
	{
		return 1;	
	}	
	if(GPIO_ReadInputDataBit(GPIOF, GPIO_Pin_1)==1)  //ÓÐº¦À¬»ø
	{
		return 2;			
	}
	if(GPIO_ReadInputDataBit(GPIOF, GPIO_Pin_2)==1)  //³øÓàÀ¬»ø
	{
		return 3;
	}
